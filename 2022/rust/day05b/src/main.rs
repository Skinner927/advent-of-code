    ...
    for line in line_iter {
        match re_instruction.captures_iter(&line).next() {
            ...
            Some(m) => {
                ...
                let tmp: Vec<char> = match stacks.get_mut(from) {
                    Some(src) => (0 .. *quantity).map(|_| src.pop().unwrap()).collect(),
                    None => panic!("Failed to get source stack: {}", &line),
                };

                if let Some(dst) = stacks.get_mut(to) {
                    tmp.iter().rev().for_each(|v| dst.push(*v));
                }
